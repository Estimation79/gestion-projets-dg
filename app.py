...
def update_sidebar_with_stats():
    """Affiche les statistiques dans la sidebar - VERSION COMPLÈTE"""
    ...
    try:
        ...
        st.sidebar.markdown("---")
        st.sidebar.markdown("<h3 style='text-align:center;color:var(--primary-color-darkest);'>📊 Stats Base</h3>", unsafe_allow_html=True)
        ...
        st.sidebar.metric("Postes", total_work_centers)
    except Exception as e:
...
def show_main_navigation():
    """Navigation principale ERP avec gestion des pages"""
    
    # GESTION NAVIGATION ET REDIRECTIONS
    if st.session_state.get('page_redirect'):
        page_to_show_val = st.session_state.page_redirect
        ...
        pages_menu = {
            ...
            "🤝 CRM": "crm_page" if CRM_AVAILABLE else None,
            "👥 Employés": "employees_page" if EMPLOYEES_AVAILABLE else None,
            "🏪 Fournisseurs": "fournisseurs_page" if FOURNISSEURS_AVAILABLE else None,
            "📑 Formulaires": "formulaires_page" if FORMULAIRES_AVAILABLE else None,
            "🤖 Assistant IA": "assistant_ia_page" if ASSISTANT_IA_AVAILABLE else None,
            "🏭 Postes de Travail": "work_centers_page" if POSTES_AVAILABLE else None,
            "⚙️ Gammes Fabrication": "manufacturing_routes" if POSTES_AVAILABLE else None,
            "📊 Capacité Production": "capacity_analysis" if POSTES_AVAILABLE else None,
            "⏱️ TimeTracker": "timetracker_page" if TIMETRACKER_AVAILABLE else None,
        ...
        # Filtrer les pages disponibles
        available_pages = {k: v for k, v in pages_menu.items() if v is not None}
        
        sel_page_key = st.sidebar.radio("Navigation:", list(available_pages.keys()), key="main_nav_radio")
...
def show_liste_projets():
    ...
    if gestionnaire.projets:
        ...
        if projets_filtres:
            ...
            # Sélection d'un projet pour les actions
            projet_ids = [p.get('id') for p in projets_filtres]
            selected_project_id = st.selectbox(
                "Sélectionner un projet pour les actions:",
                options=[""] + projet_ids,
                format_func=lambda pid: "Choisir un projet..." if pid == "" else f"#{pid} - {next((p.get('nom_projet', 'N/A') for p in projets_filtres if p.get('id') == pid), 'N/A')}",
                key="project_action_select"
            )
            
            if selected_project_id:
                selected_project = next((p for p in projets_filtres if p.get('id') == selected_project_id), None)
                ...
                with action_col2:
                    if st.button("✏️ Modifier", use_container_width=True):
                        st.session_state.edit_project_data = selected_project
    ...
    # Affichage du formulaire de création si demandé
    ...
    # Affichage de la confirmation de suppression si demandé
...
def convertir_pieds_pouces_fractions_en_valeur_decimale_fallback(mesure_str):
    """Fonction de fallback pour la conversion si le module principal n'est pas disponible"""
    try:
        # Pattern pour pieds et pouces: 5' 8 1/2"
        import re
        mesure_str = str(mesure_str).strip()
        
        # Pattern complet pour pieds et pouces avec fractions
        pattern = r"(\d+)'\s*(\d+)?\s*(\d+/\d+)?\""
        match = re.match(pattern, mesure_str)
        
        if match:
            pieds = int(match.group(1))
            pouces = int(match.group(2)) if match.group(2) else 0
            fraction_str = match.group(3) if match.group(3) else "0/1"
            
            # Conversion de la fraction
            if '/' in fraction_str:
                num, den = map(int, fraction_str.split('/'))
                fraction_pouces = num / den
            else:
                fraction_pouces = 0
            
            # Conversion totale en pieds décimaux
            total_pouces = pouces + fraction_pouces
            total_pieds = pieds + (total_pouces / 12.0)
            
            return total_pieds
        else:
            # Essayer de convertir directement en float
            return float(mesure_str)
    except:
        return 0.0

def convertir_imperial_vers_metrique_fallback(mesure_imperial):
    """Fonction de fallback pour la conversion impérial vers métrique"""
    try:
        valeur_pieds = convertir_pieds_pouces_fractions_en_valeur_decimale_fallback(mesure_imperial)
        # 1 pied = 0.3048 mètres
        return valeur_pieds * 0.3048
    except:
        return 0.0

# Fonctions de fallback si les modules ne sont pas disponibles
if not ERP_DATABASE_AVAILABLE:
    def convertir_pieds_pouces_fractions_en_valeur_decimale(mesure_str):
        return convertir_pieds_pouces_fractions_en_valeur_decimale_fallback(mesure_str)
    
    def convertir_imperial_vers_metrique(mesure_imperial):
        return convertir_imperial_vers_metrique_fallback(mesure_imperial)

# Fonction utilitaire pour nettoyer et valider les données de projet
def nettoyer_donnees_projet(projet_data):
    """Nettoie et valide les données d'un projet"""
    if not isinstance(projet_data, dict):
        return {}
    
    # Nettoyage des champs texte
    for field in ['nom_projet', 'description', 'client_nom_cache', 'client_legacy']:
        if field in projet_data and projet_data[field]:
            projet_data[field] = str(projet_data[field]).strip()
    
    # Validation des champs numériques
    try:
        if 'prix_estime' in projet_data:
            prix_str = str(projet_data['prix_estime']).replace('
...
# --- END OF FILE app.py - VERSION PORTAIL INTÉGRÉ + ERP PRODUCTION DG INC. COMPLET ---, '').replace(',', '').replace(' ', '')
            projet_data['prix_estime'] = float(prix_str) if prix_str else 0.0
    except (ValueError, TypeError):
        projet_data['prix_estime'] = 0.0
    
    try:
        if 'bd_ft_estime' in projet_data:
            projet_data['bd_ft_estime'] = float(projet_data['bd_ft_estime']) if projet_data['bd_ft_estime'] else 0.0
    except (ValueError, TypeError):
        projet_data['bd_ft_estime'] = 0.0
    
    return projet_data

# Fonction pour générer un rapport de projet
def generer_rapport_projet(projet_data):
    """Génère un rapport détaillé d'un projet"""
    if not projet_data:
        return "Aucune donnée de projet disponible"
    
    rapport = f"""
# 📊 Rapport de Projet #{projet_data.get('id', 'N/A')}

## 📋 Informations Générales
- **Nom du Projet:** {projet_data.get('nom_projet', 'N/A')}
- **Client:** {projet_data.get('client_nom_cache', 'N/A')}
- **Statut:** {projet_data.get('statut', 'N/A')}
- **Priorité:** {projet_data.get('priorite', 'N/A')}
- **Type de Tâche:** {projet_data.get('tache', 'N/A')}

## 📅 Planning
- **Date de Début:** {projet_data.get('date_soumis', 'N/A')}
- **Date de Fin Prévue:** {projet_data.get('date_prevu', 'N/A')}
- **Budget Temps (BD-FT):** {projet_data.get('bd_ft_estime', 'N/A')} heures

## 💰 Finances
- **Prix Estimé:** {format_currency(projet_data.get('prix_estime', 0))}

## 📝 Description
{projet_data.get('description', 'Aucune description disponible')}

## 🔧 Opérations
"""
    
    operations = projet_data.get('operations', [])
    if operations:
        rapport += f"Total: {len(operations)} opération(s)\n\n"
        for i, op in enumerate(operations, 1):
            rapport += f"{i}. **{op.get('description', 'N/A')}**\n"
            rapport += f"   - Poste: {op.get('poste_travail', 'Non assigné')}\n"
            rapport += f"   - Temps estimé: {op.get('temps_estime', 0)}h\n"
            rapport += f"   - Statut: {op.get('statut', 'N/A')}\n\n"
    else:
        rapport += "Aucune opération définie\n\n"
    
    rapport += f"""
## 📦 Matériaux
"""
    
    materiaux = projet_data.get('materiaux', [])
    if materiaux:
        rapport += f"Total: {len(materiaux)} matériau(x)\n\n"
        total_cost = 0
        for i, mat in enumerate(materiaux, 1):
            cost = (mat.get('quantite', 0) or 0) * (mat.get('prix_unitaire', 0) or 0)
            total_cost += cost
            rapport += f"{i}. **{mat.get('designation', 'N/A')}**\n"
            rapport += f"   - Code: {mat.get('code', 'N/A')}\n"
            rapport += f"   - Quantité: {mat.get('quantite', 0)} {mat.get('unite', '')}\n"
            rapport += f"   - Prix unitaire: {format_currency(mat.get('prix_unitaire', 0))}\n"
            rapport += f"   - Coût total: {format_currency(cost)}\n\n"
        
        rapport += f"**Coût total matériaux: {format_currency(total_cost)}**\n\n"
    else:
        rapport += "Aucun matériau défini\n\n"
    ...
    if employes_assignes:
        rapport += f"""
## 👥 Équipe Assignée
Total: {len(employes_assignes)} employé(s) assigné(s)

"""
        for emp_id in employes_assignes:
            rapport += f"- Employé ID: {emp_id}\n"
    
    return rapport

# Fonction pour exporter les données en JSON
def exporter_donnees_json(data, nom_fichier="export_erp"):
    ...
    try:
        ...
        export_data = {
            ...
            "version": "ERP Production DG Inc. v2.0",
            "data": data
        }
...
# Fonction pour valider la cohérence des données
def valider_coherence_donnees():
    """Valide la cohérence des données dans l'ERP"""
    erreurs = []
    avertissements = []
            ...
            for projet in projets:
                ...
                try:
                    ...
                    if date_fin < date_debut:
                        erreurs.append(f"Projet #{projet.get('id')}: Date de fin antérieure à la date de début")
                    
                    # Vérifier si le projet est en retard
                    if date_fin < datetime.now() and projet.get('statut') not in ['TERMINÉ', 'ANNULÉ']:
                        avertissements.append(f"Projet #{projet.get('id')}: En retard (date de fin passée)")
                        
                except (ValueError, TypeError):
                    erreurs.append(f"Projet #{projet.get('id')}: Dates invalides")
                
                # Vérifier les prix
                try:
                    prix = float(str(projet.get('prix_estime', 0)).replace('
...
# --- END OF FILE app.py - VERSION PORTAIL INTÉGRÉ + ERP PRODUCTION DG INC. COMPLET ---, '').replace(',', ''))
                    if prix < 0:
                        erreurs.append(f"Projet #{projet.get('id')}: Prix négatif")
                except (ValueError, TypeError):
                    erreurs.append(f"Projet #{projet.get('id')}: Prix invalide")
    
    except Exception as e:
        erreurs.append(f"Erreur lors de la validation: {str(e)}")
    
    return erreurs, avertissements

# Fonction pour optimiser les performances
def optimiser_performances_db():
    """Optimise les performances de la base de données"""
    try:
        if ERP_DATABASE_AVAILABLE and 'erp_db' in st.session_state:
            db = st.session_state.erp_db
            
            # Exécuter VACUUM pour optimiser la base
