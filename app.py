...
class GestionnaireProjetSQL:
    ...
    def modifier_id_projet(self, ancien_id, nouveau_id):
        """
        Modifie l'ID d'un projet existant et met à jour toutes les références
        
        Args:
            ancien_id: ID actuel du projet
            nouveau_id: Nouvel ID souhaité
        ...
        try:
            ...
            if self.check_project_id_exists(nouveau_id):
                return False
                
            # Commencer une transaction pour assurer la cohérence
            # Mettre à jour les références dans les tables liées d'abord
            
            # 1. Mettre à jour project_assignments
            self.db.execute_update(
                "UPDATE project_assignments SET project_id = ? WHERE project_id = ?",
                (nouveau_id, ancien_id)
            )
            
            # 2. Mettre à jour operations
            self.db.execute_update(
                "UPDATE operations SET project_id = ? WHERE project_id = ?",
                (nouveau_id, ancien_id)
            )
            
            # 3. Mettre à jour materials
            self.db.execute_update(
                "UPDATE materials SET project_id = ? WHERE project_id = ?",
                (nouveau_id, ancien_id)
            )
            
            # 4. Mettre à jour time_entries si la table existe
            try:
                self.db.execute_update(
                    "UPDATE time_entries SET project_id = ? WHERE project_id = ?",
                    (nouveau_id, ancien_id)
                )
            except Exception:
                pass  # Table peut ne pas exister selon la version
                
            # 5. Mettre à jour formulaires si ils référencent le projet
            try:
                self.db.execute_update(
                    "UPDATE formulaires SET project_id = ? WHERE project_id = ?",
                    (nouveau_id, ancien_id)
                )
            except Exception:
                pass  # Table peut ne pas exister
                
            # 6. Finalement, mettre à jour la table projects
            self.db.execute_update(
                "UPDATE projects SET id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                (nouveau_id, ancien_id)
            )
            
            # Ajuster next_id si nécessaire
            if nouveau_id >= self.next_id:
                self.next_id = nouveau_id + 1
                
            return True
            
        except Exception as e:
            st.error(f"Erreur modification ID projet: {e}")
            return False
...
def render_edit_project_form(gestionnaire, crm_manager, project_data):
    """Formulaire d'édition de projet - VERSION COMPLÈTE CORRIGÉE avec modification ID"""
    gestionnaire_employes = st.session_state.gestionnaire_employes
    ...
    with st.form("edit_form", clear_on_submit=True):
        ...
        # NOUVEAU : Section modification ID du projet
        st.markdown("#### 🆔 Modification du Numéro de Projet")
        
        current_project_id = project_data.get('id')
        
        modify_id_choice = st.radio(
            "Voulez-vous modifier le numéro de ce projet ?",
            ["🔒 Conserver le numéro actuel", "✏️ Modifier le numéro"],
            help="Attention: Modifier l'ID peut affecter les références dans d'autres modules."
        )
        
        new_project_id = None
        if modify_id_choice == "✏️ Modifier le numéro":
            new_project_id = st.number_input(
                "Nouveau numéro de projet:",
                min_value=1,
                max_value=999999,
                value=current_project_id,
                step=1,
                help="Entrez un nouveau numéro unique pour ce projet."
            )
            
            # Vérification en temps réel si le nouvel ID existe (sauf si c'est le même)
            if new_project_id and new_project_id != current_project_id:
                if gestionnaire.check_project_id_exists(new_project_id):
                    st.error(f"❌ Le projet #{new_project_id} existe déjà ! Choisissez un autre numéro.")
                else:
                    st.success(f"✅ Le numéro #{new_project_id} est disponible.")
            elif new_project_id == current_project_id:
                st.info(f"ℹ️ Même numéro que l'actuel (#{current_project_id})")
        else:
            st.info(f"🔒 Le numéro restera: **#{current_project_id}**")
        ...
        if submit:
            if not nom or (not selected_entreprise_id_form and not client_nom_direct_form):
                ...
                st.error("Date fin < date début.")
            elif modify_id_choice == "✏️ Modifier le numéro" and new_project_id and new_project_id != current_project_id and gestionnaire.check_project_id_exists(new_project_id):
                st.error("Nouveau numéro de projet invalide ou déjà existant.")
            else:
                ...
                # NOUVEAU : Gestion du changement d'ID
                final_project_id = current_project_id
                if modify_id_choice == "✏️ Modifier le numéro" and new_project_id and new_project_id != current_project_id:
                    # Modifier l'ID du projet
                    if gestionnaire.modifier_id_projet(current_project_id, new_project_id):
                        final_project_id = new_project_id
                        st.success(f"✅ Numéro de projet modifié: #{current_project_id} → #{new_project_id}")
                    else:
                        st.error("❌ Erreur lors de la modification du numéro de projet")
                        return

                # Mise à jour du projet
                if gestionnaire.modifier_projet(final_project_id, update_data):
                    ...
                    if employes_assignes:
                        ...
                        for emp_id in project_data.get('employes_assignes', []):
                            if emp_id not in employes_assignes:
                                ...
                                if employe:
                                    projets_existants = employe.get('projets_assignes', [])
                                    # Retirer l'ancien ID et ajouter le nouveau si nécessaire
                                    if current_project_id in projets_existants:
                                        projets_existants.remove(current_project_id)
                                    if final_project_id not in projets_existants and emp_id in employes_assignes:
                                        projets_existants.append(final_project_id)
                                    gestionnaire_employes.modifier_employe(emp_id, {'projets_assignes': projets_existants})
                        ...
                        for emp_id in employes_assignes:
                            if emp_id not in project_data.get('employes_assignes', []):
                                ...
                                if employe:
                                    projets_existants = employe.get('projets_assignes', [])
                                    if final_project_id not in projets_existants:
                                        projets_existants.append(final_project_id)
                                        gestionnaire_employes.modifier_employe(emp_id, {'projets_assignes': projets_existants})

                    # Message de succès adapté
                    if final_project_id != current_project_id:
                        st.success(f"✅ Projet #{final_project_id} (anciennement #{current_project_id}) modifié avec succès !")
                    else:
                        st.success(f"✅ Projet #{final_project_id} modifié avec succès !")
