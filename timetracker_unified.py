# timetracker_unified.py - Système de Punch Complet pour ERP Production DG Inc.
# VERSION COMPLÈTE CORRIGÉE - Pointage Simple + Pointage sur Opérations + Support Tâches BT
# Utilise directement erp_database.py pour un punch simple et efficace
# CORRIGÉ : Support complet du pointage granulaire sur opérations des Bons de Travail
# AVEC CORRECTIONS : get_punch_history() supporte les tâches BT + Méthodes de diagnostic
...
class TimeTrackerUnified:
    """
    Système de punch unifié avec support des opérations
    - Mode simple : pointage sur projets
    - Mode avancé : pointage sur opérations spécifiques des BT
    Interface unifiée pour pointage employés
    VERSION CORRIGÉE : Support complet des tâches BT depuis formulaire_lignes
    AVEC DIAGNOSTIC : Méthodes de débogage intégrées
    """
    
    def __init__(self, db):
        self.db = db
        logger.info("TimeTracker Unifié Complet initialisé")
    
    # =========================================================================
    # MÉTHODES CORE DE POINTAGE - MODE SIMPLE (PROJETS)
    # =========================================================================
    ...
    def get_active_punch(self, employee_id: int) -> Optional[Dict]:
        """Récupère le pointage actif d'un employé (mode simple)"""
        try:
            query = '''
    ...
    # =========================================================================
    # MÉTHODES POUR OPÉRATIONS - MODE AVANCÉ (CORRIGÉES)
    # =========================================================================
    ...
    def get_available_operations_hierarchical(self) -> Dict[str, List[Dict]]:
        ...
        CORRIGÉ : Inclut les tâches des BT depuis formulaire_lignes
        """
        try:
    ...
    def get_operation_info(self, operation_id: int) -> Optional[Dict]:
        ...
        CORRIGÉ : Support des tâches BT (ID > 100000)
        """
        try:
    ...
    # =========================================================================
    # MÉTHODES DE CONSULTATION - AVEC CORRECTIONS
    # =========================================================================
    ...
    def get_punch_history(self, employee_id: int = None, days: int = 7) -> List[Dict]:
        ...
        CORRIGÉ : Récupère correctement les pointages sur tâches BT depuis formulaire_lignes
        """
        try:
    ...
    def get_active_employees(self) -> List[Dict]:
        """Récupère les employés avec pointage actif (mode simple)"""
        try:
            query = '''
    ...
    def get_active_employees_with_operations(self) -> List[Dict]:
        """Récupère les employés avec pointage actif sur opérations (mode avancé)"""
        try:
            query = '''
                SELECT DISTINCT e.id, e.prenom || ' ' || e.nom as name, 
                       e.poste, p.nom_projet, te.punch_in,
    ...
    # =========================================================================
    # MÉTHODES DE DIAGNOSTIC AJOUTÉES
    # =========================================================================
...
# =========================================================================
# INTERFACE STREAMLIT PRINCIPALE - MODE SIMPLE
# =========================================================================

def show_timetracker_unified_interface():
    """Interface principale du TimeTracker unifié - MODE SIMPLE ET AVANCÉ"""
    ...
    st.markdown("### ⏱️ TimeTracker Unifié - Pointage Employés")
    
    # Sélecteur de mode
    col_mode1, col_mode2 = st.columns([3, 1])
    
    with col_mode1:
        st.markdown("**Mode de pointage :**")
    
    with col_mode2:
        use_operation_mode = st.toggle("🔧 Mode Opérations", value=False, 
                                      help="Active le pointage granulaire sur les opérations des Bons de Travail")
    
    if use_operation_mode:
        # MODE AVANCÉ : Pointage sur opérations
        st.info("🔧 **Mode Opérations Activé** - Pointage granulaire sur les opérations spécifiques")
        
        # Onglets pour mode opérations
        ...
        # MODE SIMPLE : Pointage sur projets
        st.info("📋 **Mode Simple Activé** - Pointage sur projets généraux")
        
        # Onglets pour mode simple
...
# =========================================================================
# INTERFACES MODE SIMPLE
# =========================================================================

def show_punch_interface(tt):
    """Interface de pointage simple sur projets"""
    
    st.markdown("#### 🕐 Pointage sur Projets")
...
def show_history_interface(tt):
    """Interface d'historique des pointages mode simple"""
    ...
    for h in history:
        ...
        # Déterminer l'opération pour affichage
        operation_display = "Projet général"
        if h.get('operation_description'):
            if h.get('sequence_number'):
                operation_display = f"Op.{h['sequence_number']:02d}: {h['operation_description']}"
            else:
                operation_display = h['operation_description']
            
            # Ajouter le poste de travail si disponible
            if h.get('work_center_name'):
                operation_display += f" ({h['work_center_name']})"
        
        df_data.append({
            'ID': h['id'],
            'Statut': status,
            'No. Projet': h.get('project_id', 'N/A'),
            'Nom Projet': h['nom_projet'] or 'N/A',
            'No. BT': h.get('bt_numero', ''),
            'Opération': operation_display,
            'Date': h['date_travail'],
...
def show_statistics_interface(tt):
    """Interface de statistiques mode simple"""
...
# =========================================================================
# INTERFACES MODE AVANCÉ - OPÉRATIONS
# =========================================================================
...
def get_timetracker_redirect_to_bt():
    """Fonction de compatibilité - pas utilisée dans version complète"""
    return False
