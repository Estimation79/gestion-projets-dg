...
def render_bons_commande_tab(gestionnaire):
    ...
    col_action1, col_action2, col_action3, col_action4 = st.columns(4)
    with col_action1:
        if st.button("➕ Nouveau Bon de Commande", use_container_width=True):
            st.session_state.form_action = "create_bon_commande"
    with col_action2:
        if st.button("📋 Liste Complète", use_container_width=True):
            st.session_state.form_action = "list_bon_commande"
    with col_action3:
        if st.button("🔄 Depuis BA", use_container_width=True):
            st.session_state.form_action = "convert_ba_to_bc"  # Déjà implémenté !
    with col_action4:
        if st.button("📊 Suivi Livraisons", use_container_width=True):
    ...
    with col_action6:
        if st.button("📥 Réception Marchandises", use_container_width=True):
            st.session_state.form_action = "reception_marchandises"
    with col_action7:
        if st.button("📋 Templates BC", use_container_width=True):
...
def render_bon_commande_list(gestionnaire):
    ...
    if not bons_commande:
        ...
        with col_start1:
            if st.button("➕ Créer Premier BC", use_container_width=True):
                st.session_state.form_action = "create_bon_commande"
                st.rerun()
        with col_start2:
            if st.button("🔄 Convertir depuis BA", use_container_width=True):
                st.session_state.form_action = "convert_ba_to_bc"
                st.rerun()
        return
    ...
    if bons_filtres:
        ...
        with col_action5:
            if st.button("📥 Marquer Reçu", use_container_width=True):
                if bc_selectionne:
...
def render_delivery_tracking(gestionnaire):
    ...
    with col_rapid1:
        if st.button("📦 Marquer Plusieurs comme Expédiés", use_container_width=True):
            count = 0
            for appro in appros_filtres:
    ...
    with col_rapid2:
        if st.button("✅ Marquer Arrivées Aujourd'hui", use_container_width=True):
            count = 0
            for appro in appros_filtres:
                try:
                    date_prevue = datetime.strptime(appro['date_livraison_prevue'], '%Y-%m-%d').date()
    ...
    with col_rapid3:
        if st.button("📧 Relancer Fournisseurs en Retard", use_container_width=True):
            if retards:
        ...
        if livraison_selectionnee:
            ...
            if lignes_bc:
                ...
                with st.form(f"reception_form_{livraison_id}"):
                    ...
                    with col_submit1:
                        submit_reception = st.form_submit_button("✅ Valider la Réception", use_container_width=True)
                    
                    with col_submit2:
                        submit_reception_partielle = st.form_submit_button("📦 Réception Partielle", use_container_width=True)
...
{notes_reception}"""
                            ...
                            col_next1, col_next2 = st.columns(2)
                            with col_next1:
                                if st.button("📋 Retour Liste BC", use_container_width=True):
                                    st.session_state.form_action = "list_bon_commande"
                                    st.rerun()
                            with col_next2:
                                if st.button("📥 Autre Réception", use_container_width=True):
                                    st.rerun()
